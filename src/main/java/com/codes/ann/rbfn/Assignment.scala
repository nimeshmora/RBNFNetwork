...
object RBNFAssignment {

  val conf = new SparkConf().setMaster("local").setAppName("RBNFAssignment")
  ...
  def evaluateRBFN(weightsSets:Array[DenseMatrix[Double]], centersSets:Array[Array[Vector]], zigmasSets:Array[DenseVector[Double]], testData:RDD[Vector]): Double ={
    ...
    println("Number Right: " + numRight)
    println("Number of Inputs: " + input.rows.toDouble)
